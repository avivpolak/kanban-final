...
const sortObj = (obj) => Object.entries(obj).reduce((acc, [key, val]) => ({ ...acc, [key]: val.sort() }), {})

const localStorage = async () => await page.evaluate(() => Object.assign({}, window.localStorage))
...
const addTasksAndTest = async (startTingIndex = 0) =>
    await full4s.asyncForEach(lists, async (listItem, index) => {
        ...
        const newTaskText = await (await newTask.getProperty('innerText')).jsonValue()
...
const emptyTasks = sortObj(tasksTypes.reduce((acc, curr) => ({ ...acc, [curr]: [] }), {}))

const fullTasks = sortObj(tasksTypes.reduce((acc, curr, index) => ({ ...acc, [curr]: [`Tasks #${index}`] }), {}))

let wasInPut = false

describe(projectName, () => {
    beforeAll(async () => {
        browser = await puppeteer.launch({ headless: false, slowMo: 100 }) //change to false if you want to view the page
        ...
        page.on('request', async (req) => {
            ...
            if (req.method() === 'GET') {
                ...
                req.respond({
                    ...getResponse(),
                    body: JSON.stringify({
                        tasks: tasksTypes.reduce((acc, curr, _, all) => ({ ...acc, [curr]: all }), { name: 'test' }),
                    }),
                })
                wasInPut = true
            }
        ...
        await full4s.asyncForEach(lists, async (listItem, i) => {
            ...
            switch (i) {
                case 0:
                    ...
                    break
                case 1:
                    expectedData[tasksTypes[i]] = [...Array(2)].map((_, i) => `Tasks #${i}`)
    ...
    test('User should be able to edit task with double click and the new task data will be saved in the local storage', async () => {
        ...
        const getTaskText = async () => (await firstTask.getProperty('innerText')).jsonValue()
